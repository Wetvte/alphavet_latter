...
/* Создание контроллера клиента */

function createApp() {
    /* Создание экземпляра приложения Express*/
    const app = express(); // Контроллер
    ...
    app.get("/", async (req, res) => {
        ...
        if (user && user["status"] === "Authorized") {
            ...
            if (update_response.status == 200) {
                ...
                // Редирект
                res.redirect(server_url+`/cabinet?user_id=${user["id"]}`);
                return;
            }
    ...
    // Коллбэк для авторизации
    app.get("/service_auth_callback", async (req, res) => {
        // Получаем токен
        const { session_token } = req.query;
        // Если он передан, сохраняем и переходим к автовходу
        if (session_token) {
            res.cookie("session_token", session_token);
            res.redirect(server_url+"/login");
        }
        // Иначе ответ с не очень статусом
    ...
    app.get("/login", async (req, res) => {
        ...
        // так как ему нужно автоматом войти, и он сохранился анонимным, обрабатываем автовход

        // Получаем параметр "type" из query‑строки URL
        const { type } = req.query;
        // Цель визита страницы
        ...
        let user = await get_redis_user(session_token);
        // Если нет данных
        if (!user || !user["status"] || user["status"] === "Unknown" || !user["login_token"]) {
            // Авто-входу не быть
            await delete_session(req, res);
            return res.redirect(server_url+"/");
        }
        ...
        if (target == "check" || target == "check_and_confirm") {
            ...
            if (target == "check_and_confirm") {
                return res.redirect(server_url+"/login?target=confirm");
            }
        ...
        if (target == "confirm" || target == "check_and_confirm") {
            ...
            const confirm_response = await post_service_confirm_auth_request(user["login_token"]);
            ...
            user = await create_redis_session(session_token, {
                ...
                nickname: confirm_response.data["nickname"],
    ...
    app.get("/primary", async (req, res) => {
        ...
        res.status(200).send(generate_site_page("primary", "Главная", { ...req.query }));
    });
    ...
    app.get("/cabinet", async (req, res) => {
        ...
        res.status(200).send(generate_site_page("cabinet", title, { ...req.query, viewer: user }));
    });
    // Просмотр дисциплины
    app.get("/discipline", async (req, res) => {
        ...
        if (!discipline_id) return res.redirect(server_url+"/primary");
        ...
        res.status(200).send(generate_site_page("discipline", "Дисциплина", { ...req.query, viewer: user }));
    });
    // Просмотр теста
    app.get("/test", async (req, res) => {
        ...
        if (!test_id) return res.redirect(server_url+"/primary");
        ...
        // Отправляем файл страницы 
        res.status(200).send(generate_site_page("test", try_id ? "Попытка" : "Тест", { ...req.query, viewer: user }));
    });
    ...
    app.get("/logout", async (req, res) => {
        ...
        if (all === "true") {
            await get_logout_request(user["refresh_token"])
        }
    ...
    app.get("/sub", async (req, res) => {
        ...
        const user = await get_redis_user(session_token);
        if (!user) return res.redirect(server_url+"/");
...
async function refresh_user(user, session_token) {
    const refresh_request = await get_refresh_request(user["refresh_token"]);
...
async function post_service_auth_request(service_type, session_token, login_token) {
    return await axios.post(process.env.AUTHORIZATION_MODULE_URL + "/auth/service",
        {
            service_type,
            session_token,
            callback_url: process.env.SERVER_URL + "/service_auth_callback",
        },
        {
            headers: {
...
async function post_service_confirm_auth_request(login_token) {
    return await axios.get(process.env.AUTHORIZATION_MODULE_URL + "/auth/confirm",
        {
            headers: {
                "Authorization": `Bearer ${login_token}`,
                "Content-Type": "application/json"
...
async function get_refresh_request(refresh_token) {
    return await axios.get(process.env.AUTHORIZATION_MODULE_URL + "/refresh",
        {
            headers: {
...
async function get_logout_request(refresh_token) {
    return await axios.get(process.env.AUTHORIZATION_MODULE_URL + "/logout/all",
        {
            headers: {
                "Authorization": `Bearer ${refresh_token}`,
                "Content-Type": "application/json"
...
function generate_site_page(name, title, settings) {
  ...
  ${sectionsJS}
    ...
    // также данные о том, кто просматривает страницу

    return page;
}

/* Создание серверов и подключение к Redis */
...
// Порт берётся из переменных окружения
const controllers_count = process.env.SERVERS_COUNT;
...
let controllers = [];
console.log(`Запускаем контроллеры (${controllers_count})`);
app = createApp();
for (let i = 0; i < controllers_count; i++) {
    const cport = +port + (+i + 1)
    const controller = app.listen(cport, (err) => {
        if (err) {
            console.error(`Ошибка на порту ${cport}:`, err);
            return;
        }
        console.log(`Сервер контроллера клиента запущен на ${host}:${cport}`);
    });
    // Обработка ошибок сервера
    controller.on('error', (err) => {
        console.error(`Сервер на порту ${cport} упал:`, err);
    });
...
process.on('SIGINT', () => {
    console.log('> Получен сигнал остановки. Закрываем сервера контроллеров.');

    controllers.forEach((server, index) => {
        server.close(() => {
            console.log(`Контроллер ${+port + +index + 1} остановлен.`);
        });
    });

    // Завершаем процесс после остановки всех серверов
    setTimeout(() => {
